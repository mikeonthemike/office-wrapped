...
// v1.0.4 - Improved export-friendly title handling for mobile
...
export const SlideCard = forwardRef<HTMLDivElement, SlideCardProps>(
  ({ slide, activeSlide, totalSlides, isTransitioning, children }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className="slide-card relative w-full max-w-4xl aspect-[3/4] sm:aspect-[4/3] bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col border-2 border-slate-700"
      >
        {/* Card header - compact padding for more content space */}
        <div className="slide-header bg-gradient-to-r from-blue-600 to-purple-600 text-white p-3 sm:p-4 flex justify-between items-center flex-shrink-0">
          <div className="flex-1 min-w-0">
            <div className="uppercase tracking-widest text-[10px] sm:text-xs font-semibold opacity-90">
              Year in Review
            </div>
            <h2 className="slide-title text-lg sm:text-3xl font-black leading-tight">{slide.title}</h2>
          </div>
        ...
        <div
          className={`slide-content flex-1 p-3 sm:p-6 bg-gradient-to-b from-slate-800 to-slate-900 relative transition-opacity duration-300 overflow-hidden ${
